...
class InvalidUsageHandlerImpl @Inject constructor(
    ...
    override fun handle(dreamSender: DreamSender<*>, optionalCommandMeta: Optional<CommandMeta>, commandInput: CommandInput) {
        ...
        if (commandPathMetas.isEmpty()) {
            this.messageConfig.pathNotFound.send(bukkitSender.handler, MapBuilder<String, Any>()
        ...
        this.messageConfig.usage.send(bukkitSender.handler, MapBuilder<String, Any>()
            .put("label", "/" + commandMeta.commandContext.name)
            .put("description", commandMeta.commandContext.description)
            .build())

        for (commandPathMeta in commandPathMetas) {
            this.messageConfig.usagePath.send(bukkitSender.handler, MapBuilder<String, Any>()
